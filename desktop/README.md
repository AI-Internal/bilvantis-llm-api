# BilvantisLLM-API Desktop

Lean Electron menu-bar app for [bilvantisllmapi](https://github.com/AI-Internal/bilvantis-llm-api):
...
npm run desktop:dist:win    # Windows → desktop/dist-electron/BilvantisLLM-API-…-x64 installer
```
...
the root, which does both. `BILVANTIS_REPO=/path/to/checkout` overrides the
client/server source location if you ever need a different one.

UI iteration helpers (dev only): `BILVANTIS_SHOT=1 npx electron .` captures the
popover + dashboard to /tmp and quits; `BILVANTIS_SHOT=hold` pins the popover
open for real-screen captures.

## Notes

- DB + config live in `~/Library/Application Support/BilvantisLLM-API/` (macOS) /
  `%APPDATA%/BilvantisLLM-API/` (Windows).
