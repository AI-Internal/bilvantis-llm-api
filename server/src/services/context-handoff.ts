import type { ChatMessage } from '@bilvantisllmapi/shared/types.js';
...
export function getContextHandoffMode(): ContextHandoffMode {
  const raw = process.env.BILVANTISLLMAPI_CONTEXT_HANDOFF?.trim().toLowerCase();
...
export function maybeInjectContextHandoff(params: {
  ...
  const alreadyPresent = messages.some(m => {
    ...
    return text.startsWith('BilvantisLLM-API context handoff:');
  });
