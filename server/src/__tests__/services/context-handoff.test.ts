...
beforeEach(() => {
  _clearStoreForTesting();
  delete process.env.BILVANTISLLMAPI_CONTEXT_HANDOFF;
});
...
describe('getContextHandoffMode', () => {
  ...
  it('reads on_model_switch', () => {
    process.env.BILVANTISLLMAPI_CONTEXT_HANDOFF = 'on_model_switch';
    expect(getContextHandoffMode()).toBe('on_model_switch');
  });

  it('treats unknown value as off', () => {
    process.env.BILVANTISLLMAPI_CONTEXT_HANDOFF = 'always';
    expect(getContextHandoffMode()).toBe('off');
  });
});
...
describe('maybeInjectContextHandoff — on_model_switch', () => {
  ...
  it('injects handoff when model switches', () => {
    ...
    expect(injected!.content).toContain('BilvantisLLM-API context handoff:');
  ...
  it('does not duplicate if handoff system message already present as plain string', () => {
    ...
    const existingHandoff = msg('system', 'BilvantisLLM-API context handoff: prior injection');
    const result = maybeInjectContextHandoff({
      mode,
      sessionKey: 'sess1',
  ...
  it('does not duplicate if handoff system message present as array-content (OpenCode format)', () => {
    ...
    const arrayHandoff = {
      role: 'system',
      content: [{ type: 'text', text: 'BilvantisLLM-API context handoff: prior injection' }],
    } as any;
    const result = maybeInjectContextHandoff({
      mode,
      sessionKey: 'sess1',
  ...
  it('inserts handoff after existing system messages', () => {
    ...
    expect(result.messages[1].content).toContain('BilvantisLLM-API context handoff:');
  });
